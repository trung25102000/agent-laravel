...
- Ngày: 2026-06-01
- Task: 074-ux-writing-review-and-content-optimization
- Loại: decision
- Nội dung: Đặt locale mặc định của `seo-web-app` là `vi` và bổ sung `lang/vi/auth.php`, `lang/vi/validation.php` để auth error/validation message đồng nhất với public copy tiếng Việt.
- Lý do: Task 074 yêu cầu toàn bộ text hiển thị cho người dùng phải tự nhiên, dễ hiểu và không lẫn tiếng Anh mặc định của Laravel; chỉ sửa view mà bỏ qua validation/auth message sẽ làm trải nghiệm bị gãy.
- Ảnh hưởng: Public site, form và login/register hiển thị nhất quán tiếng Việt hơn; test feature cần assert message mới thay vì fallback English.

- Ngày: 2026-06-01
- Task: 073-homepage-ux-simplification-and-conversion-refactor
- Loại: decision
- Nội dung: Rút Home Page về 8 lớp nội dung chính: hero, pain points, services, why choose us, portfolio, process, feedback và CTA cuối; đồng thời bỏ pricing/FAQ khỏi Home và giữ `data-hero-visuals` ở dạng static board thay vì rotator nhiều scene.
- Lý do: Audit mới cho thấy vấn đề nằm ở overload và lặp thông điệp, không phải thiếu nội dung; giữ static hero board giúp giảm nhiễu nhưng vẫn tương thích với visual system/tests hiện có.
- Ảnh hưởng: Home Page ngắn hơn, CTA rõ hơn, luồng khách mới trực tiếp hơn; các trang con vẫn giữ phần detail sâu như pricing, FAQ, services và portfolio.
...
- Ảnh hưởng: Mỗi app có composer/npm/env/migration/test riêng; root repo vẫn là control plane cho agent/tasks/.agents/context.
...
- Ảnh hưởng: Các lần chạy sau chỉ cần thêm task vào `/tasks` và dùng prompt trong `/.codex/prompts`.
